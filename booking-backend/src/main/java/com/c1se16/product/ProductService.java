...
import com.c1se16.user.response.ItemResponse;
...
public class ProductService {
    ...
    public ItemResponse findItemById(String id) {
        return this.itemRepository.findDetailItemById(id)
                .map(item -> {
                    Product product = item.getProduct();
                    ItemResponse itemResponse = new ItemResponse();
                    itemResponse.setItemId(item.getId());
                    itemResponse.setQuantity(item.getQuantity());
                    itemResponse.setAvatar(product.getAvatar());
                    itemResponse.setCode(product.getCode());
                    itemResponse.setName(product.getName());
                    itemResponse.setShortDescription(product.getShortDescription());
                    itemResponse.setStatus(product.getStatus());
                    UserDTO userDTO = this.objectMapper.convertValue(item.getOwner(), UserDTO.class);
                    itemResponse.setOwner(userDTO);
                    return itemResponse;
                })
